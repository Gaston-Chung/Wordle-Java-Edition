...
public class ReverseWordlePage extends JFrame
{
   //------------ JComps ------------
    ...
    //Other
    private static HashSet <String> all5LetterEnglishWords = new HashSet <String>();
    public static boolean playerVictory;
    int a, x = 250; int b, y = 100; int c = 75;

    //Reverse Exclusive
    public static boolean reverseEnabled, endlessReverseEnabled;
    public ArrayList <String> previousLetters = new ArrayList<String>();
    public boolean firstTime;
    public boolean answer = true;
    public ArrayList <String> temporaryArray = new ArrayList<String>();
    public Color orangeColor = new Color(255, 160, 36);
    public String problemLetter;
    ...
    public ReverseWordlePage()
    {
        Word.generateRandomWord();
        currentWord = Word.getCurrentWordString();
        ...
        roundsPanel.setBounds(45, 115, 160, 40);
        roundsPanel.setBackground(new Color(250, 75, 75));
        roundsPanel.setBorder(BorderFactory.createLineBorder(Color.BLACK, 3));
        
        roundsLabel = new JLabel("Round " + currentRound);
        roundsLabel.setFont(new Font("Helvetica", Font.BOLD, 25));
        roundsPanel.add(roundsLabel);
        background.add(roundsPanel);
        ...
        errorLabel.setFont(new Font("Helvetica", Font.BOLD, 23));
    ...
    public class TextFieldListener implements ActionListener
    {
        ...
        public void actionPerformed(ActionEvent e)
        {
            ...
            if (attemptsCounter == 5)
            {
                ...
                new javax.swing.Timer(1500, evt ->
                {
                    ...
                    if ((statusArray[0] == true) && (statusArray[1] == true) && (statusArray[2] == true) && (statusArray[3] == true) && (statusArray[4] == true))
                    {
                        ...
                        else if (endlessReverseEnabled == true)
                        {
                            //Rounds Incrementation
                            currentRound++;
                    ...
                    //If the player reached all 5 attempts and they DIDN'T get it right
                    else
                    {
                        //Setting High Score (if they broke it, which always happens when you compile and run the program)
                        if (currentRound > HomePage.classicHighScore)
                        {
                            HomePage.classicHighScore = currentRound;
                            HomePage.newHighScoreFlag = true;
                        }

                        //Set flag to player lost and then make game results page (to enable the screen that shows they lost)
                        playerVictory = false;
                        new GameResultsPage();
                        HomePage.newHighScoreFlag = false;

                        //Resetting Flags
                        reverseEnabled = false;
                        endlessReverseEnabled = false;

                        //Empty out universalPreviousWords
                        HomePage.universalPreviousWords.clear();
                    }
            ...
            enterText.setText(""); //Clear the box after every attempt
            ...
            if (guessedWord.length() != 5)
            {
                ...
                new javax.swing.Timer(2500, evt ->
                {
                    ((Timer) evt.getSource()).stop();
            ...
            else
            {
                ...
                {
                    String nonLettersDetectedMsg = "Error: Word contains characters that aren't letters.";
                ...
                {
                    if (all5LetterEnglishWords.contains(guessedWord) == false)
                    {
                        String notEnglishWordMsg = "Error: Word is either profanity, not a commonly used word, or not English at all.";
                    ...
                    {
                        if (checkLetterHistory(guessedWord) == false)
                        {
                            String previouslyUsedLetterMsg = "Error: " + guessedWord + " contains the wrong letter " +  problemLetter + " which was used in a previous submission.";
                            errorLabel.setText(previouslyUsedLetterMsg);
                            errorLabel.setVisible(true);
                        ...
                        {
                            if ( (previousWords.contains(guessedWord)) || ( (endlessReverseEnabled == true) && (HomePage.universalPreviousWords.contains(guessedWord)) && (Word.masterListString.contains(guessedWord) == false) ) )
                            {
                            ...
                            else
                            {
                                ...
                                if (endlessReverseEnabled == true)
                                {
                                    HomePage.universalPreviousWords.add(guessedWord);
                                }
                            }
                        }
                    }
    ...
    public boolean checkLetterHistory(String someWord)
    {
        //Break someWord into its letters
        for (int i = 0; i < 5; i++)
        {
            char currentCharOld = someWord.charAt(i);
            String currentChar = (String.valueOf(currentCharOld)).toUpperCase();
            temporaryArray.add(currentChar); //All 5 letters have been added to the temp array
        }

        //Check if any letters in tempArray are in previousLetters
        for (int i = 0; i < 5; i++)
        {
            //Only WRONG letters cannot be used again, so make sure to only add WRONG letters to previousLetters
            if ( previousLetters.contains( temporaryArray.get(i) ) && ( currentWord.contains(temporaryArray.get(i)) == false ) ) 
            {
                problemLetter = temporaryArray.get(i);
                answer = false;
                temporaryArray.clear();
                break;
            }
            //Do NOT add the good letters to the previousLetters ArrayList yet, we need to finish checking all 5 letters in tempArray FIRST
        }

        //This goes first in order to prevent an error
        if (answer == false)
        {
            answer = true;
            return false;
        ...
        //Now it's all good to accept the most recent word, and we can add the letters to previousLetters now
        previousLetters.add(temporaryArray.get(0));
        previousLetters.add(temporaryArray.get(1));
        previousLetters.add(temporaryArray.get(2));
        previousLetters.add(temporaryArray.get(3));
        previousLetters.add(temporaryArray.get(4));
        temporaryArray.clear();
    ...
    public void checkWordCorrectness(String playersWord)
    {
        ...
        for (int i = 0; i < 5; i++)
        {
            for (int j = 0; j < 5; j++)
            {
                ...
                char currentWordCharTracker = Word.getCurrentWordString().charAt(i);
        ...
        //If the player guessed the word correctly - For some reason this only brings you to the GameResults page if you put it here and not the makePanels method even though there's no reason for that
        if ((statusArray[0] == true) && (statusArray[1] == true) && (statusArray[2] == true) && (statusArray[3] == true) && (statusArray[4] == true))
        {
            submitAnswerButton.setEnabled(false);
            hintButton.setEnabled(false);
    ...
    public class HintButtonListener implements ActionListener
    {
        ...
        public void actionPerformed(ActionEvent e)
        {
            if (e.getSource() == hintButton)
            {
                ...
                String answer = Word.getCurrentWordString();
    ...
    public void makeLetterPanels(String text)
    {
        ...
        for (int j = 0; j<5; j++)
        {
            ...
            {
                tilesArray[j].setBackground(orangeColor);
                ...
                if (((text.charAt(j)) == 'Q') && (qLock == false)) { qLabel.setBackground(orangeColor); }
                if (((text.charAt(j)) == 'W') && (wLock == false)) { wLabel.setBackground(orangeColor); }
                if (((text.charAt(j)) == 'E') && (eLock == false)) { eLabel.setBackground(orangeColor); }
                if (((text.charAt(j)) == 'R') && (rLock == false)) { rLabel.setBackground(orangeColor); }
                if (((text.charAt(j)) == 'T') && (tLock == false)) { tLabel.setBackground(orangeColor); }
                if (((text.charAt(j)) == 'Y') && (yLock == false)) { yLabel.setBackground(orangeColor); }
                if (((text.charAt(j)) == 'U') && (uLock == false)) { uLabel.setBackground(orangeColor); }
                if (((text.charAt(j)) == 'I') && (iLock == false)) { iLabel.setBackground(orangeColor); }
                if (((text.charAt(j)) == 'O') && (oLock == false)) { oLabel.setBackground(orangeColor); }
                if (((text.charAt(j)) == 'P') && (pLock == false)) { pLabel.setBackground(orangeColor); }
                if (((text.charAt(j)) == 'A') && (aLock == false)) { aLabel.setBackground(orangeColor); }
                if (((text.charAt(j)) == 'S') && (sLock == false)) { sLabel.setBackground(orangeColor); }
                if (((text.charAt(j)) == 'D') && (dLock == false)) { dLabel.setBackground(orangeColor); }
                if (((text.charAt(j)) == 'F') && (fLock == false)) { fLabel.setBackground(orangeColor); }
                if (((text.charAt(j)) == 'G') && (gLock == false)) { gLabel.setBackground(orangeColor); }
                if (((text.charAt(j)) == 'H') && (hLock == false)) { hLabel.setBackground(orangeColor); }
                if (((text.charAt(j)) == 'J') && (jLock == false)) { jLabel.setBackground(orangeColor); }
                if (((text.charAt(j)) == 'K') && (kLock == false)) { kLabel.setBackground(orangeColor); }
                if (((text.charAt(j)) == 'L') && (lLock == false)) { lLabel.setBackground(orangeColor); }
                if (((text.charAt(j)) == 'Z') && (zLock == false)) { zLabel.setBackground(orangeColor); }
                if (((text.charAt(j)) == 'X') && (xLock == false)) { xLabel.setBackground(orangeColor); }
                if (((text.charAt(j)) == 'C') && (cLock == false)) { cLabel.setBackground(orangeColor); }
                if (((text.charAt(j)) == 'V') && (vLock == false)) { vLabel.setBackground(orangeColor); }
                if (((text.charAt(j)) == 'B') && (bLock == false)) { bLabel.setBackground(orangeColor); }
                if (((text.charAt(j)) == 'N') && (nLock == false)) { nLabel.setBackground(orangeColor); }
                if (((text.charAt(j)) == 'M') && (mLock == false)) { mLabel.setBackground(orangeColor); }
            }
            ...
            if (statusArray[j] == true)
            {
                ...
                if ((text.charAt(j)) == 'Q') { qLabel.setBackground(orangeColor); qLock = true;}
                if ((text.charAt(j)) == 'W') { wLabel.setBackground(orangeColor); wLock = true;}
                if ((text.charAt(j)) == 'E') { eLabel.setBackground(orangeColor); eLock = true; }
                if ((text.charAt(j)) == 'R') { rLabel.setBackground(orangeColor); rLock = true;}
                if ((text.charAt(j)) == 'T') { tLabel.setBackground(orangeColor); tLock = true;}
                if ((text.charAt(j)) == 'Y') { yLabel.setBackground(orangeColor); yLock = true;}
                if ((text.charAt(j)) == 'U') { uLabel.setBackground(orangeColor); uLock = true;}
                if ((text.charAt(j)) == 'I') { iLabel.setBackground(orangeColor); iLock = true;}
                if ((text.charAt(j)) == 'O') { oLabel.setBackground(orangeColor); oLock = true;}
                if ((text.charAt(j)) == 'P') { pLabel.setBackground(orangeColor); pLock = true;}
                if ((text.charAt(j)) == 'A') { aLabel.setBackground(orangeColor); aLock = true;}
                if ((text.charAt(j)) == 'S') { sLabel.setBackground(orangeColor); sLock = true;}
                if ((text.charAt(j)) == 'D') { dLabel.setBackground(orangeColor); dLock = true;}
                if ((text.charAt(j)) == 'F') { fLabel.setBackground(orangeColor); fLock = true;}
                if ((text.charAt(j)) == 'G') { gLabel.setBackground(orangeColor); gLock = true;}
                if ((text.charAt(j)) == 'H') { hLabel.setBackground(orangeColor); hLock = true;}
                if ((text.charAt(j)) == 'J') { jLabel.setBackground(orangeColor); jLock = true;}
                if ((text.charAt(j)) == 'K') { kLabel.setBackground(orangeColor); kLock = true;}
                if ((text.charAt(j)) == 'L') { lLabel.setBackground(orangeColor); lLock = true;}
                if ((text.charAt(j)) == 'Z') { zLabel.setBackground(orangeColor); zLock = true;}
                if ((text.charAt(j)) == 'X') { xLabel.setBackground(orangeColor); xLock = true;}
                if ((text.charAt(j)) == 'C') { cLabel.setBackground(orangeColor); cLock = true;}
                if ((text.charAt(j)) == 'V') { vLabel.setBackground(orangeColor); vLock = true;}
                if ((text.charAt(j)) == 'B') { bLabel.setBackground(orangeColor); bLock = true;}
                if ((text.charAt(j)) == 'N') { nLabel.setBackground(orangeColor); nLock = true;}
                if ((text.charAt(j)) == 'M') { mLabel.setBackground(orangeColor); mLock = true;}
            }
    ...
    {
        SwingUtilities.invokeLater(ReverseWordlePage::new);
    }
}